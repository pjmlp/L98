/* ASTNotEqual.java : AST node for the not equal operator.
 ...
 */
public class ASTNotEqual extends ASTBinOp {
  ...
  */
  public ASTNotEqual (int line, ASTExp left, ASTExp right) { super (line, left, right);}
  ...
  public Type transverse (Environ env, CompilerError err, CodeGenerator gen, int nesting) {
    ...
    if (left != null && right != null) {
      if (!left.equals (right)) {
        err.message(getLine () + ": Comparing invalid types");
        result = null;
      }
      else {
        result = new TypeBool ();
