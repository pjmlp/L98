/* ASTDiv.java : AST node for division operations.
...
/**
 * AST node for division operations.
 */
public class ASTDiv extends ASTBinOp {
  ...
  */
  public ASTDiv (int line, ASTExp left, ASTExp right) { super (line, left, right);}
  ...
  public Type transverse (Environ env, CompilerError err, CodeGenerator gen, int nesting) {
    ...
    if (left != null && right != null) {
      if (!(left instanceof TypeInt) || !(right instanceof TypeInt)) {
        err.message(getLine() + ": division applied to invalid types");
        result = null;
      }
      else {
        result = new TypeInt ();
