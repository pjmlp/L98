/* ASTAnd.java : AST node for the logical and operation
...
/**
 * AST node for the logical and operation
 */
public class ASTAnd extends ASTBinOp {
   ...
   */
  public ASTAnd (int line, ASTExp left, ASTExp right) { super (line, left, right);}
  ...
  public Type transverse (Environ env, CompilerError err, CodeGenerator gen, int nesting) {
    ...
    if (left != null && right != null) {
      if (!(left instanceof TypeBool) || !(right instanceof TypeBool)) {
        err.message(getLine() + ": Invalid type used with the and operator");
        result = null;
      }
      else {
        result = new TypeBool ();
