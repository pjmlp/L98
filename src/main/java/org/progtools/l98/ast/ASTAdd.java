/* ASTAdd.java : AST node for the addition operation
...
/**
 * AST node for the addition operation.
 */
public class ASTAdd extends ASTBinOp {
   ...
   */
  public ASTAdd (int line, ASTExp left, ASTExp right) { super (line, left, right);}
  ...
  public Type transverse (Environ env, CompilerError err, CodeGenerator gen, int nesting) {
    ...
    if (left != null && right != null) {
      if (!(left instanceof TypeInt) || !(right instanceof TypeInt)) {
        err.message(getLine() + ": Invalid type used in the addition");
        result = null;
      }
      else {
        result = new TypeInt ();
