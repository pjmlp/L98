/* ASTUMinus.java : AST node for the sign inversion operator.
...
/**
 * AST node for the sign inversion operator.
 */
public class ASTUMinus extends ASTUnOp {
 /**
  * @param line where the expression was found.
  * @param node expression to invert the sign
  */
  public ASTUMinus (int line, ASTExp node) { super (line, node);}
  ...
  public Type transverse (Environ env, CompilerError err, CodeGenerator gen, int nesting) {
    ...
    if (node != null) {
      if (!(node instanceof TypeInt)) {
        err.message(getLine() + ": Invalid type applied to the sign inversion operator");
        result = null;
      }
      else {
        result = new TypeInt ();
