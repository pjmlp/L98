/* ASTGreaterOrEqual.java : AST node for the greater or equal than operator.
...
/**
 * AST node for the greater or equal than operator.
 */
public class ASTGreaterOrEqual extends ASTBinOp {
  ...
  */
  public ASTGreaterOrEqual (int line, ASTExp left, ASTExp right) { super (line, left, right);}
  ...
  public Type transverse (Environ env, CompilerError err, CodeGenerator gen, int nesting) {
    ...
    if (left != null && right != null) {
      if (!(left instanceof TypeInt) || !(right instanceof TypeInt)) {
        err.message(getLine() + ": Invalid type in >= expression");
        result = null;
      }
      else {
        result = new TypeBool ();
