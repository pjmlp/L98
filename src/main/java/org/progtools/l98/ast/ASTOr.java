/* ASTOr.java : AST node for the or operator.
...
public class ASTOr extends ASTBinOp {
  ...
  */
  ...
  public ASTOr (int line, ASTExp left, ASTExp right) { super (line, left, right);}
  ...
  public Type transverse (Environ env, CompilerError err, CodeGenerator gen, int nesting) {
    ...
    if (left != null && right != null)
      if (!(left instanceof TypeBool) || !(right instanceof TypeBool)) {
        err.message(getLine () + ": Invalid type applied to or operator");
        result = null;
      }
      else {
        result = new TypeBool ();
