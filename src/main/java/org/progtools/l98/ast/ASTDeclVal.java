/* ASTDeclVal.java : AST node for value declarations
...
/**
 * AST node for value declarations
 */
public class ASTDeclVal extends ASTDecl {
  /**
   * Identifier's name.
   */
  ...
  * @param id   value's name.
  * @param type return type.
  * @param exp initialization expression
  */
  public ASTDeclVal (int line, String id, Type type, ASTExp exp) {
    super (line);
    m_id = id;
    m_type = type;
    m_exp = exp;
  ...
  private boolean typeCheck (Type expType, CompilerError err) {
    ...
    if (expType != null && !m_type.equals (expType)) {
       err.message (getLine() + ": Invalid expression type assigned to constant");
       retValue = false;
    }
  ...
  public int transverse (Environ env, CompilerError err, CodeGenerator gen, int nesting, int index) {
     ...
     try {
	   ...
	   valAccess = new Frame (nesting, index);
          ...
          // Generates the code for storing the constant's value
          valAccess.genStoreAccess (gen, nesting);
	  attr = new Attributes (m_type, valAccess, false, false);
       }
     }
     catch (IOException e) {
